...
## Corpus Check
- 185 files · ~66,422 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1316 nodes · 3106 edges · 116 communities (72 shown, 44 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 174 edges (avg confidence: 0.67)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `de3600e0`
...
Cohesion: 0.14
Nodes (25): _already_reminded_today(), cron_trigger(), date, post, Request, Session, Manually trigger the same automatic sweep (also runs on its own daily)., The external daily trigger — cron, a systemd timer, Task Scheduler. The in-… (+17 more)

### Community 1 - "test_pass_read_spot.py"
Cohesion: 0.17
Nodes (11): IssuePassRequest, LotCheckResult, PassListItem, PassRead, BaseModel, RenewPassRequest, The spot number rides on every pass payload — ticket, list, verify., test_pass_read_carries_spot_number() (+3 more)
...
Cohesion: 0.06
Nodes (64): AuthStatus, get_current_user(), Session, Admin or manager — the money tier. An attendant's job is the front desk: issue,…, require_admin(), require_manager(), create_access_token(), decode_access_token() (+56 more)

### Community 5 - "book/page.tsx"
Cohesion: 0.12
Nodes (32): BookPage(), PASS_TYPES, PayWay, expiryPhrase(), LotCheckInner(), PAYMENT_LABEL, shortDate(), STATUS_STYLE (+24 more)
...
### Community 7 - "monthly-customers/page.tsx"
Cohesion: 0.12
Nodes (19): currency(), MonthlyCustomer, MonthlyCustomersPage(), STATUS_LABEL, ComingSoon(), InfoTip(), AddStaffCard(), Role (+11 more)
...
### Community 9 - "cn"
Cohesion: 0.12
Nodes (29): Badge(), badgeVariants, Command(), CommandDialog(), CommandEmpty(), CommandGroup(), CommandInput(), CommandItem() (+21 more)

### Community 10 - "move_spot"
Cohesion: 0.22
Nodes (10): clear_overstay(), lot_state(), move_spot(), post, Session, Staff dealt with the squatter — put the spot back to normal., Cashier override: move a truck to a specific free spot. Login-gated by the…, MoveSpotRequest (+2 more)
...
### Community 13 - "api.ts"
Cohesion: 0.06
Nodes (49): ACTION_STYLE, AuditLogPage(), fmt(), styleFor(), STATE_CLASS, currency(), GROUPS, MorningReportPage() (+41 more)
...
Cohesion: 0.22
Nodes (8): CompanyProfilePage(), fmtDate(), fmtDateTime(), money(), money2(), STATUS_STYLE, StatusBadge(), CompanyProfile

### Community 18 - "test_renewal.py"
Cohesion: 0.15
Nodes (23): add_months(), date, apply_renewal(), cancel_pass(), get_pass(), issue_pass(), post, Session (+15 more)

### Community 19 - "components.json"
Cohesion: 0.09
Nodes (21): aliases, components, hooks, lib, ui, utils, iconLibrary, menuAccent (+13 more)

### Community 20 - "_issue_pass_and_payment"
Cohesion: 0.23
Nodes (12): _find_or_create_vehicle(), _issue_pass_and_payment(), Reuse the company's existing vehicle instead of inserting a new row every time…, Shared by the normal Issue Pass endpoint and the Stripe finalize/webhook…, _issue(), Company + vehicle de-duplication on issue (the freeform-name bug)., test_find_or_create_vehicle_reuses_by_truck_number(), test_issue_dedupes_company_and_vehicle() (+4 more)
...
### Community 24 - "routers/insights.py"
Cohesion: 0.12
Nodes (14): Settings, get_db(), Session, health(), lifespan(), The lot, painted by query. `expiring` = holder's last day is today or tomorrow…, DashboardStats, BaseModel (+6 more)
...
Cohesion: 0.21
Nodes (12): main(), client(), db(), engine(), _fresh_engine(), fixture, Session, Test fixtures: an isolated in-memory SQLite database per test. The money-path… (+4 more)

### Community 33 - "issue/page.tsx"
Cohesion: 0.11
Nodes (17): IDENTIFIER_FIELD, PASS_TYPES, PAY_CHOICES, PayChoice, VEHICLE_TYPES, PaymentRequestQR(), Card(), CardAction() (+9 more)

### Community 34 - "passes.py"
Cohesion: 0.19
Nodes (17): _expiration_for(), list_passes(), _monthly_rate_for(), _months_between(), _price_for(), Whole calendar months between two dates, rounding UP for any partial overage…, The established PER-MONTH rate — never multiplied by month count. An existing…, _validate_weekly_span() (+9 more)
...
Cohesion: 0.08
Nodes (29): inter, jetbrainsMono, metadata, viewport, CommandPalette(), AppShell(), isPublicPath(), MobileNav() (+21 more)
...
Cohesion: 0.14
Nodes (26): ensure_spots(), free_spot_count(), pick_free_spot(), FCFS with a memory: longest-vacant first (NULLS FIRST — a never-used spot is…, Idempotent: rows exist for 1..capacity, and exactly those are active. Shrinking…, _issue(), A spot is held while its pass is live: daily through expiry day, monthly…, test_cancelled_pass_frees_immediately() (+18 more)
...
Cohesion: 0.20
Nodes (14): company_profile(), Everything about one company in one place: totals, trucks, recent passes and…, _find_company(), _lookup_monthly_rate(), Company, The company's established per-month rate, or None if it has no monthly plan on…, Match a company by name case-insensitively, ignoring surrounding whitespace —…, _issue() (+6 more)

### Community 50 - "core/spots.py"
Cohesion: 0.10
Nodes (24): _held_spot_ids(), holding_filter(), _live_window_filter(), move_pass_to_spot(), MoveError, Session, Spot inventory. State is DERIVED: a spot is free iff no live pass holds it.…, A spot the system may hand to the NEXT customer: active, not held by a live… (+16 more)
...
Cohesion: 0.24
Nodes (9): business_now(), plaza_tz(), datetime, What day is it *at the plaza*. Two separate bugs live here if you get this…, Wall-clock time at the plaza, naive — stored as-is so `date(paid_at)` in SQL is…, Break-glass password reset — run ON THE SERVER, no login required. For the one…, The plaza's day. Two bugs lived here, and both only showed themselves in the…, test_business_now_is_wall_clock_time_at_the_plaza() (+1 more)

### Community 61 - "business_today"
Cohesion: 0.14
Nodes (19): business_today(), date, The plaza's current calendar day. Use this everywhere `date.today()` was used,…, monthly_renewal_list(), Every monthly customer with how their renewal stands, soonest first. Shared…, Kiritimati (UTC+14) and Niue (UTC-11) are 25 hours apart, so their calendar…, The invariant the money depends on: the day a payment is recorded under is the…, test_a_payment_is_stamped_with_the_plazas_day() (+11 more)
...
Cohesion: 0.29
Nodes (10): Reminder, _do_send(), MonthlyCustomer, Build + send (SMS if configured) + record one reminder. Shared by the manual…, _reminder_message(), BaseModel, ReminderCustomer, RemindersOverview (+2 more)
...
### Community 87 - "payments/page.tsx"
Cohesion: 0.24
...
Cohesion: 0.13
Nodes (26): currency(), PassesPage(), Field(), PAY_CHOICES, PayChoice, PassTicket(), PAY_CHOICES, PayChoice (+18 more)

### Community 92 - "seed_demo.py"
Cohesion: 0.24
Nodes (6): ParkingPass, _issue(), _phone(), Seed realistic demo data into the dev database so every screen populates. Run…, seed(), Base
...
- **Why does `business_today()` connect `business_today` to `test_reminders_sweep.py`, `test_pass_read_spot.py`, `PassType`, `routers/reports.py`, `test_morning_report.py`, `models/__init__.py`, `ensure_spots`, `test_insights.py`, `reminders.py`, `clock.py`, `test_spots_endpoint.py`, `enums.py`, `routers/insights.py`, `test_report_occupied.py`, `seed_demo.py`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `PassType` connect `PassType` to `test_pass_read_spot.py`, `passes.py`, `test_lot_check.py`, `test_morning_report.py`, `ensure_spots`, `test_insights.py`, `test_company_profile.py`, `test_renewal.py`, `clock.py`, `_issue_pass_and_payment`, `test_spots_endpoint.py`, `enums.py`, `routers/insights.py`, `test_report_occupied.py`, `seed_demo.py`, `business_today`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `PaymentMethod` connect `PassType` to `test_reminders_sweep.py`, `test_pass_read_spot.py`, `test_lot_check.py`, `Payment`, `test_morning_report.py`, `ensure_spots`, `MonthlyCustomer`, `test_insights.py`, `test_company_profile.py`, `test_renewal.py`, `clock.py`, `_issue_pass_and_payment`, `test_spots_endpoint.py`, `enums.py`, `routers/insights.py`, `test_report_occupied.py`, `seed_demo.py`, `business_today`?**
